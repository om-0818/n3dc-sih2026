//#region node_modules/.nitro/vite/services/ssr/assets/access-DhNv62cr.js
function pinchWidth(caze) {
	return caze.fireRoadActualM < 6 ? Math.min(4.8, caze.fireRoadActualM) : caze.fireRoadActualM;
}
function evaluateAccess(caze) {
	...
	return [
		{
			...
			pass: roadOk,
			note: highRise ? "High-rise (≥ 24 m): 6.0 m clear carriageway to the compound." : "Low-rise access road per UDCPR table 6-A."
		},
		{
			id: "pinch",
			code: "NBC-4.6",
		...
		},
		{
			...
			pass: hydrantPass,
			note: hydrantPass ? "Hose lay along the clearance route to hydrant H-04." : "Path is blocked at the pinch, so hose length is measured via the failed route."
		},
		{
			id: "ramp",
			code: "UDCPR-6.10",
...
function accessVerdict(checks) {
	...
	return {
		pass: failed === 0,
		failed,
		title: failed === 0 ? "Clearance route viable" : `${failed} UDCPR / NBC check${failed === 1 ? "" : "s"} failed`
	};
}
/** Single calculation shared by the summary badge, 3D path, and simulation banner. */
function simulateAccess(caze) {
	...
	return {
		...
		pathEnd: verdict.pass ? .97 : .62
	};
}
//#endregion
export { simulateAccess as t };
